...
public:
	inline void initial(ModelParams& model_params, HyperParams& hyper_params, AlignedMemoryPool *mem = NULL) {
		...
		for (int idx = 0; idx < maxsize; idx++) {
			...
			_bi_rnn_concat[idx].init(hyper_params.rnnHiddenSize * 2, -1, mem);
		}
		...
		_grnn_left.init(&model_params.grnn_left_layer, hyper_params.dropOut, true, mem);
		_grnn_right.init(&model_params.grnn_right_layer, hyper_params.dropOut, false, mem);
		...
		_pooling_concat.init(hyper_params.hiddenSize * 4 * 3, -1, mem);
		...
		_concat.init(hyper_params.hiddenSize * 4 * 3 + hyper_params.hiddenSize, -1, mem);
